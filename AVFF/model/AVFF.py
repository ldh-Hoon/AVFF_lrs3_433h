...
class AVFF(nn.Module):
    def __init__(self, 
                ...
                v_num_patches=196,
                a_num_patches=48,
                marlin_path = "/workspace/AVFF/pretrained_models/MARLIN/marlin_vit_base_ytf.full.pt",
                audioMAE_path = '/workspace/AVFF/pretrained_models/AudioMAE/pretrained.pth'):
    ...
    def forward_loss(self, L_c, v_encoded, a_encoded, v_rec, a_rec, i_v, i_a, 
    ...
    def forward_with_mask(self, video, audio):
        ...
        L_G, L_D, _ = self.forward_loss(L_c, v_encoded, a_encoded, v_rec, a_rec, i_v, i_a)
    ...
    def contrastive_loss(self, v, a, tau=0.1):
        ...
        video_mean = v.mean(dim=2).view(B * num_slices, -1)
        audio_mean = a.mean(dim=2).view(B * num_slices, -1)
        logits = torch.matmul(video_mean, audio_mean.T) / tau

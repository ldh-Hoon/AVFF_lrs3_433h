...
class AVFF(nn.Module):
    ...
    def forward_loss_nomask(self, L_c, v, a, v_encoded, a_encoded, v_rec, a_rec, i_v, i_a,
        ...
        # Reconstruction Loss
        loss_rec = self.reconstruction_loss_nomask(a, a_rec, v, v_rec)
    ...
    def forward_with_nomask(self, video, audio):
        ...
        L_G, L_D, (loss_c, loss_rec, loss_adv) = self.forward_loss_nomask(L_c, video, audio, v_encoded, a_encoded, v_rec, a_rec, i_v, i_a)
        
        return v_rec, a_rec, v_a, a_v, i_v, i_a, (L_G, L_D, loss_c, loss_rec, loss_adv, v_encoded, a_encoded)
    ...
    def forward_with_mask(self, video, audio):
        ...
        # forward_loss에 L_c와 인코딩된 텐서들을 전달
        L_G, L_D, (loss_c, loss_rec, loss_adv) = self.forward_loss(L_c, video, audio, v_encoded, a_encoded, v_rec, a_rec, i_v, i_a)
    ...
    def reconstruction_loss_nomask(self, x_a, x_a_rec, x_v, x_v_rec):
        if x_a_rec.shape != x_a.shape:
            x_a_rec = x_a_rec.squeeze(1)
        loss_a = F.mse_loss(x_a_rec, x_a)

        # Video
        loss_v = F.mse_loss(x_v_rec, x_v)

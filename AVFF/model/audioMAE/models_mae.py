...
# reference
# from https://github.com/facebookresearch/AudioMAE
...
from .pos_embed import get_2d_sincos_pos_embed, get_2d_sincos_pos_embed_flexible, get_1d_sincos_pos_embed_from_grid
...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward_decoder_no_mask(self, x):
        ...
        if self.decoder_mode != 0:
            B, L, D = x.shape
            #x = x[:, 1:, :]  # 첫 번째 토큰(cls 토큰)을 제외하고 나머지 토큰 사용
            
            if self.use_custom_patch:
                # 패치 크기 및 스트라이드에 따라 재구성
        ...
        if self.decoder_mode != 0:
            if self.use_custom_patch:
                ...
                pred = pred[:, :101, :, :]  # 마지막 패치 제거
                pred = pred.reshape(B, 1212, 256)
            else:
                pred = pred
        else:
            pred = pred[:, 1:, :]  # cls 토큰 제거
